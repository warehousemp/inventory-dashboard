(() => {
  ...
  const navigation = [
    {
      ...
      links: [
        { label: 'System / Admin Tools', href: 'users.html', roles: ['ADMIN', 'SYSTEM_ADMIN'] }
      ]
    },
    {
      label: 'Accounting',
    ...
    },
    {
      ...
      links: [
        { label: 'Projects Command Center', href: 'projects-command.html', roles: ['PROJECTS', 'ADMIN', 'SYSTEM_ADMIN'] }
      ]
    },
    {
      label: 'Service',
      ...
      links: [
        ...
        { label: 'Route Sales Opportunities', href: 'rso-tracking.html', roles: ['ADMIN', 'SYSTEM_ADMIN'] },
  ...
  function renderShell(container) {
    container.innerHTML = `
      <div class="shared-nav-topline">
        <a class="shared-nav-brand" href="index.html" aria-label="Molinari Pools main menu">
          <span class="shared-nav-brand-mark">MP</span>
          <span>MOLINARI POOLS</span>
        </a>
        <div class="shared-nav-account">
          <span id="sharedNavUser" class="shared-nav-user">Loading menu...</span>
          <button id="sharedNavLogout" class="shared-nav-logout" type="button">Logout</button>
          <button id="sharedNavMobileToggle" class="shared-nav-mobile-toggle" type="button" aria-label="Open menu" aria-expanded="false">☰</button>
        </div>
      </div>
      <nav id="sharedNavMenubar" class="shared-nav-menubar" aria-label="Company systems navigation"></nav>
    `;
  }
  ...
  function renderMenu(role) {
    ...
    const homeActive = currentPage === '' || currentPage === 'index.html';
    const visibleSections = navigation
      .filter(section => isAllowed(section.roles, role))
      .map(section => ({
        ...section,
        links: section.links.filter(link => isAllowed(link.roles, role))
      }))
      .filter(section => section.links.length > 0);

    const sectionHtml = visibleSections.map((section, sectionIndex) => {
      const hasActivePage = section.links.some(link => link.href.toLowerCase() === currentPage);
      const links = section.links.map(link => {
        const active = link.href.toLowerCase() === currentPage;
        return `<a class="shared-nav-link${active ? ' active' : ''}" href="${escapeHtml(link.href)}"${active ? ' aria-current="page"' : ''}>${escapeHtml(link.label)}</a>`;
      }).join('');

      return `
        <div class="shared-nav-item" data-nav-section="${sectionIndex}">
          <button class="shared-nav-trigger${hasActivePage ? ' active-section' : ''}" type="button" aria-expanded="false">
            <span>${escapeHtml(section.label)}</span><span class="shared-nav-caret">▼</span>
          </button>
          <div class="shared-nav-dropdown">${links}</div>
        </div>
      `;
    }).join('');

    menubar.innerHTML = `
      <a class="shared-nav-home${homeActive ? ' active' : ''}" href="index.html"${homeActive ? ' aria-current="page"' : ''}>Main Menu</a>
      ${sectionHtml}
    `;
  ...
  function closeAllDropdowns(exceptItem = null) {
    document.querySelectorAll('.shared-nav-item.open').forEach(item => {
      if (item !== exceptItem) {
        item.classList.remove('open');
        const trigger = item.querySelector('.shared-nav-trigger');
  ...
  function bindMenuInteractions() {
    document.querySelectorAll('.shared-nav-trigger').forEach(trigger => {
      trigger.addEventListener('click', event => {
        event.stopPropagation();
        const item = trigger.closest('.shared-nav-item');
      ...
      trigger.addEventListener('keydown', event => {
        if (event.key === 'ArrowDown') {
          ...
          const firstLink = item.querySelector('.shared-nav-link');
          if (firstLink) firstLink.focus();
        }
      });
    });

    document.querySelectorAll('.shared-nav-dropdown').forEach(dropdown => {
      dropdown.addEventListener('keydown', event => {
        if (event.key === 'Escape') {
          const item = dropdown.closest('.shared-nav-item');
          item.classList.remove('open');
          const trigger = item.querySelector('.shared-nav-trigger');
  ...
  async function loadProfile() {
    ...
    navClient = window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY);
    const { data: sessionData, error: sessionError } = await navClient.auth.getSession();
    if (sessionError) throw sessionError;

    const session = sessionData?.session;
    if (!session?.user) return { session: null, profile: null };

    const { data: profile, error: profileError } = await navClient
      .from('user_profiles')
  ...
  async function logout() {
    ...
    if (navClient) await navClient.auth.signOut();
  ...
  async function initializeSharedNavigation() {
    ...
    document.body.classList.add('has-shared-navigation');
    ...
    const mobileToggle = document.getElementById('sharedNavMobileToggle');
    const menubar = document.getElementById('sharedNavMenubar');
    mobileToggle.addEventListener('click', event => {
      event.stopPropagation();
      const open = menubar.classList.toggle('mobile-open');
      mobileToggle.setAttribute('aria-expanded', String(open));
      mobileToggle.setAttribute('aria-label', open ? 'Close menu' : 'Open menu');
    });

    document.getElementById('sharedNavLogout').addEventListener('click', logout);

    try {
      const { session, profile } = await loadProfile();
      currentProfile = profile;
      const userLabel = document.getElementById('sharedNavUser');

      if (!session || !profile || profile.active !== true) {
        userLabel.textContent = 'Not signed in';
        document.getElementById('sharedNavLogout').textContent = 'Login';
        document.getElementById('sharedNavLogout').onclick = () => { window.location.href = 'index.html'; };
        renderMenu(null);
        return;
      }
      ...
      const userLabel = document.getElementById('sharedNavUser');
      if (userLabel) userLabel.textContent = 'Menu access check failed';
      renderMenu(null);
      console.error('Shared navigation initialization failed:', error);
    }
  }
